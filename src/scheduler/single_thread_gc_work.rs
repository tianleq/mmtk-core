    ...
    P: Plan<VM = VM> + PlanTraceObject<VM>,
{
    pub fn new(mmtk: &'static MMTK<VM>, worker: &mut GCWorker<VM>) -> Self {
        Self {
            ...
            sources: Vec::with_capacity(EDGES_WORK_BUFFER_SIZE),
        }
    }
    ...
    fn process_slot(&mut self, _source: Option<ObjectReference>, slot: VM::VMSlot) {
        ...
        //     let description = if crate::util::metadata::public_bit::is_public(source) {
        //         "public"
        //     } else {
        //         "private"
        //     };
    ...
    P: Plan<VM = VM> + PlanTraceObject<VM>,
{
    fn enqueue(&mut self, object: ObjectReference) {
        ...
        let mut closure = |_object: ObjectReference, slot: VM::VMSlot| {
            let Some(_) = slot.load() else { return };
            self.slots.push(slot);
            self.sources.push(Some(_object));
        };
    ...
    P: Plan<VM = VM> + PlanTraceObject<VM>,
{
    ...
    fn traverse(&mut self) {
        assert_eq!(self.slots.len(), self.sources.len());
        while let (Some(source), Some(slot)) = (self.sources.pop(), self.slots.pop()) {
            self.process_slot(source, slot);
        }
    }

    fn traverse_from_roots(&mut self, _root_slots: Vec<VM::VMSlot>) {
        unreachable!()
    }
}
