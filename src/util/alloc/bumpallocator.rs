...
impl<VM: VMBinding> Allocator<VM> for BumpAllocator<VM> {
    ...
    fn alloc_slow_once_precise_stress(
        ...
        if new_cursor > self.cursor + self.limit.as_usize() {
            ...
            // );
            // result
            self.alloc_fast_stress(size, align, offset)
        }
    }
...
impl<VM: VMBinding> BumpAllocator<VM> {
    ...
    fn acquire_block(
        ...
        if acquired_start.is_zero() {
            ...
            if !stress_test {
                ...
                self.alloc_fast(size, align, offset)
            } else {
    ...
    #[inline(always)]
    fn alloc_fast(&mut self, size: usize, align: usize, offset: isize) -> Address {
        let result = align_allocation_no_fill::<VM>(self.cursor, align, offset);
        let new_cursor = result + size;
    ...
    #[inline(always)]
    fn alloc_fast_stress(&mut self, size: usize, align: usize, offset: isize) -> Address {
        let result = align_allocation_no_fill::<VM>(self.cursor, align, offset);
        let new_cursor = result + size;
