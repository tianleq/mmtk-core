...
use crate::plan::{ThreadlocalTracedObjectType, VectorObjectQueue};
    ...
    fn trace_object<Q: ObjectQueue, const KIND: TraceKind>(
        ...
        if KIND == TRACE_KIND_DEFRAG {
            if Block::containing::<VM>(object).is_defrag_source() {
                ...
                debug_assert!(
                    false,
                    "All blocks must be defrag source when thread local gc is enabled. block: {:?} is not",
...
impl<VM: VMBinding> ImmixSpace<VM> {
    ...
    fn side_metadata_specs() -> Vec<SideMetadataSpec> {
        metadata::extract_side_metadata(&if super::BLOCK_ONLY {
            vec![
                #[cfg(feature = "thread_local_gc")]
                MetadataSpec::OnSide(Block::BLOCK_PUBLICATION_TABLE),
                #[cfg(feature = "thread_local_gc")]
                MetadataSpec::OnSide(Block::BLOCK_LINKED_LIST_PREV_TABLE),
                #[cfg(feature = "thread_local_gc")]
                MetadataSpec::OnSide(Block::BLOCK_LINKED_LIST_NEXT_TABLE),
                #[cfg(feature = "thread_local_gc")]
    ...
    pub fn defrag_headroom_pages(&self) -> usize {
        self.defrag.defrag_headroom_pages(self) + self.bytes_published.load(Ordering::Acquire)
    }
    ...
    pub fn release(&mut self, major_gc: bool) -> bool {
        ...
        if !super::BLOCK_ONLY {
            ...
            debug_assert!(
                self.reusable_blocks.len() == 0,
                "thread local gc should not use global reusable blocks list"
            );
            #[cfg(not(feature = "thread_local_gc"))]
    ...
    pub fn get_reusable_block(
        ...
        loop {
            if let Some(block) = self.reusable_blocks.pop() {
                ...
                if _owner != block.owner() {
                    continue;
                }
                #[cfg(feature = "thread_local_gc")]
                if let Some(s) = _semantic {
                    match s {
    ...
    pub fn thread_local_trace_object_without_moving(
        ...
        {
            // Here the object is private, check the owner
    ...
    pub fn thread_local_mark_lines(&self, object: ObjectReference, local_state: u8) {
        ...
        // cannot overwrite public line mark here
        Line::thread_local_mark_lines_for_object::<VM>(object, local_state);
    }

    #[cfg(feature = "thread_local_gc")]
    fn thread_local_attempt_mark(&self, object: ObjectReference, mark_state: u8) -> bool {
        unsafe {
            let old_value = VM::VMObjectModel::LOCAL_MARK_BIT_SPEC.load::<VM, u8>(object, None);
            if old_value == mark_state {
                return false;
            }
            VM::VMObjectModel::LOCAL_MARK_BIT_SPEC.store::<VM, u8>(object, mark_state, None);
            true
        }
    }
    ...
    pub fn get_next_available_lines(
        ...
        while cursor < mark_data.len() {
            ...
            #[cfg(feature = "thread_local_gc")]
            if mark != unavail_state && mark != current_state && mark != public_line_state {
                break;
            }
            cursor += 1;
        }
    ...
    pub fn publish_object(&self, object: ObjectReference) {
        ...
        if !super::BLOCK_ONLY {
            ...
            Line::publish_lines_for_object::<VM>(object, state);
        }
    ...
    pub fn get_new_object(&self, object: ObjectReference) -> ObjectReference {
        if ForwardingWord::is_forwarded::<VM>(object) {
            return ForwardingWord::thread_local_get_forwarded_object::<VM>(object);
        }
        #[cfg(debug_assertions)]
        {
            assert!(
                ForwardingWord::get_forwarding_status::<VM>(object) == 0,
...
impl<VM: VMBinding> crate::policy::gc_work::PolicyThreadlocalTraceObject<VM> for ImmixSpace<VM> {
    fn thread_local_trace_object<const KIND: TraceKind>(
        ...
        if KIND == TRACE_THREAD_LOCAL_FAST {
            ...
            self.thread_local_trace_object_without_moving(_mutator_id, object)
        } else if KIND == TRACE_THREAD_LOCAL_DEFRAG {
            if Block::containing::<VM>(object).is_defrag_source() {
    ...
    fn alloc_copy(
        ...
        #[cfg(feature = "thread_local_gc")]
        {
            if crate::util::public_bit::is_public::<VM>(_original) {
                ...
                // can only lives in a block of the same owner
