...
pub(crate) fn generate_trace_object<'a>(
    ...
    let space_field_handler = space_fields.iter().map(|f| {
        ...
        quote! {
            if self.#f_ident.in_space(__mmtk_objref) {
                return <#f_ty as PolicyTraceObject #ty_generics>::trace_object::<T, KIND>(&self.#f_ident, __mmtk_trace, __mmtk_source, __mmtk_objref, #copy, __mmtk_worker);
            }
        }
    });
    ...
    let parent_field_delegator = if let Some(f) = parent_field {
        ...
        let ref f_ty = f.ty;
        quote! {
            <#f_ty as PlanTraceObject #ty_generics>::trace_object::<T, KIND>(&self.#f_ident, __mmtk_trace, __mmtk_source, __mmtk_objref, __mmtk_worker)
        }
    } else {
        quote! {
    ...
    quote! {
        #[inline(always)]
        fn trace_object<T: crate::plan::TransitiveClosure, const KIND: crate::policy::gc_work::TraceKind>(&self, __mmtk_trace: &mut T, __mmtk_source: crate::util::ObjectReference, __mmtk_objref: crate::util::ObjectReference, __mmtk_worker: &mut crate::scheduler::GCWorker<VM>) -> crate::util::ObjectReference {
